...
export const claudeIcon = new LabIcon({
  name: 'galyleo:claude',
  svgstr: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="10" fill="#D4865A"/><path d="M32 10 L38 28 L56 28 L42 38 L48 56 L32 46 L16 56 L22 38 L8 28 L26 28 Z" fill="none" stroke="white" stroke-width="3" stroke-linejoin="round"/><circle cx="32" cy="32" r="6" fill="white"/></svg>'
});

export const codexIcon = new LabIcon({
  name: 'galyleo:codex',
  svgstr: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="10" fill="#10A37F"/><polyline points="20,22 8,32 20,42" fill="none" stroke="white" stroke-width="4.5" stroke-linecap="round" stroke-linejoin="round"/><polyline points="44,22 56,32 44,42" fill="none" stroke="white" stroke-width="4.5" stroke-linecap="round" stroke-linejoin="round"/><line x1="38" y1="16" x2="26" y2="48" stroke="white" stroke-width="4.5" stroke-linecap="round"/></svg>'
});
...
const plugin: JupyterFrontEndPlugin<void> = {
  ...
  activate: (
    ...
    const OPEN_CLAUDE = 'galyleo:open-claude';
    commands.addCommand(OPEN_CLAUDE, {
      label: 'Claude Code',
      caption: 'Open Claude Code in a terminal',
      icon: claudeIcon,
      execute: () => app.commands.execute('terminal:create-new', { initialCommand: 'claude\n' })
    });

    const OPEN_CODEX = 'galyleo:open-codex';
    commands.addCommand(OPEN_CODEX, {
      label: 'Codex',
      caption: 'Open OpenAI Codex in a terminal',
      icon: codexIcon,
      execute: () => app.commands.execute('terminal:create-new', { initialCommand: 'codex\n' })
    });

    launcher.add({ command: CREATE_NEW, category: 'Other', rank: 1 });
    launcher.add({ command: OPEN_CLAUDE, category: 'AI Tools', rank: 1 });
    launcher.add({ command: OPEN_CODEX, category: 'AI Tools', rank: 2 });
  }
};
